...
function sortByBit(arr) {
  return arr.sort(function (a, b) {
    const aBitCount = (a.toString(2).match(/1/g) || []).length;
    const bBitCount = (b.toString(2).match(/1/g) || []).length;
    if (aBitCount < bBitCount) return -1;
    if (aBitCount > bBitCount) return 1;
    if (aBitCount == bBitCount) return a - b;
  });
}
